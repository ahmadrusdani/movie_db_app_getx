...
class NowPlayingMovie {
  ...
  Future<BaseResponseList<List<MovieModel>>?> call(
      {int page = 1, bool isPreview = true}) async {
    final box = await Hive.openBox<MovieModel>('now_playing_movie');
    try {
      final result = await repository.getNowPlayingMovie(page: page);
