...
import 'upcoming_movie_test.mocks.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  late MockMovieRepository mockRepository;
  late UpcomingMovie upcomingMovie;
  ...
  setUp(() async {
    ...
    upcomingMovie = UpcomingMovie(repository: mockRepository);
    testBox = await Hive.openBox<MovieModel>('upcoming_movie');
  ...
  group('UpcomingMovie UseCase', () {
    test('should store movies and return response when successful (page 1)',
        () async {
      when(mockRepository.getUpcomingMovie(page: 1))
          .thenAnswer((_) async => testResponse);

      final result = await upcomingMovie.call(page: 1);

      expect(result, equals(testResponse));
      verify(mockRepository.getUpcomingMovie(page: 1)).called(1);
    ...
    test('should append movies without clearing box when page > 1', () async {
      ...
      when(mockRepository.getUpcomingMovie(page: 2))
          .thenAnswer((_) async => testResponse.copyWith(page: 2));

      final result = await upcomingMovie.call(page: 2);
    ...
    test('should return preview of cached movies when isPreview=true',
      ...
      final result = await upcomingMovie.call(page: 1, isPreview: true);
    ...
    test('should clear box when page=1 and new data arrives', () async {
      ...
      await upcomingMovie.call(page: 1);
