...
void main() {
  ...
  setUp(() async {
    ...
    nowPlayingMovie = NowPlayingMovie(repository: mockRepository);
    testBox = await Hive.openBox<MovieModel>('now_playing_movie');
  ...
  group('NowPlayingMovie UseCase', () {
    test('should store movies and return response when successful (page 1)',
        () async {
      when(mockRepository.getNowPlayingMovie(page: 1))
          .thenAnswer((_) async => testResponse);

      final result = await nowPlayingMovie.call(page: 1);

      expect(result, equals(testResponse));
      verify(mockRepository.getNowPlayingMovie(page: 1)).called(1);
    ...
    test('should append movies without clearing box when page > 1', () async {
      ...
      when(mockRepository.getNowPlayingMovie(page: 2))
          .thenAnswer((_) async => testResponse.copyWith(page: 2));

      final result = await nowPlayingMovie.call(page: 2);
    ...
    test('should return preview of cached movies when isPreview=true',
      ...
      final result = await nowPlayingMovie.call(page: 1, isPreview: true);
    ...
    test('should clear box when page=1 and new data arrives', () async {
      ...
      await nowPlayingMovie.call(page: 1);
